...
**Version:** v27 (nach Sprint 43 – Export & Pagination)
**Letzte Aktualisierung:** 2026-03-28
**Status:** Sprint 43 abgeschlossen, v2.8.0 deployed und verifiziert auf Produktivumgebung
...
| `/api/v1/weeklyplans/export-xlsx/` | **NEU:** Wochenpläne als XLSX exportieren |
| `/api/v1/weeklyplans/export-pdf/` | **NEU:** Wochenpläne als PDF exportieren |
| `/api/v1/groups/students/` | Schüler (CRUD) |
| `/api/v1/groups/students/export-xlsx/` | **NEU:** Schüler als XLSX exportieren |
| `/api/v1/groups/students/export-pdf/` | **NEU:** Schüler als PDF exportieren |
| `/api/v1/groups/daily-protocols/` | Tagesprotokolle (CRUD) |
| `/api/v1/groups/daily-protocols/export-xlsx/` | **NEU:** Tagesprotokolle als XLSX exportieren |
| `/api/v1/groups/daily-protocols/export-pdf/` | **NEU:** Tagesprotokolle als PDF exportieren |
...
## Sprint 43 – Export & Pagination

### Haupt-Features

Der Sprint hat eine systemweite Infrastruktur für Datenexporte und serverseitige Pagination implementiert. Dies umfasst ein generisches Backend-Mixin und wiederverwendbare Frontend-Komponenten, die in den Ansichten für Schüler, Tagesprotokolle und Wochenpläne integriert wurden.

### Backend

*   **`ExportMixin`:** Ein neues Mixin in `core/mixins_export.py` stellt `export-xlsx` und `export-pdf` Aktionen für ViewSets bereit. Es nutzt `openpyxl` für XLSX und `reportlab` für PDF.
*   **`StandardPagination`:** Eine neue Paginierungsklasse in `core/pagination.py` erweitert die API-Antworten um `total_pages`, `current_page` und `page_size`.

### Frontend

*   **`<ExportButtons />`:** Eine wiederverwendbare Komponente für ein Dropdown-Menü mit Exportoptionen.
*   **`<Pagination />`:** Die bestehende Komponente wurde zur Nutzung der neuen API-Metadaten aktualisiert und in allen relevanten Tabellenansichten integriert.
